...
const resetGraph = () => {
  ...
  graphOptions.layouts[0].from = 'left';
  graphOptions.layouts[0].max_per_width = 300;
  graphOptions.layouts[0].min_per_height = 40;
  graphOptions.layouts[0].max_per_height = 200;
  graphOptions.defaultNodeWidth = 100;
  graphOptions.defaultNodeHeight = 30;
  graphOptions.defaultJunctionPoint = 'tb';
  seeksRelationGraph.value && seeksRelationGraph.value.setOptions(graphOptions as any, (graphInstance) => {
    showSeeksGraph();
  })
}
const showSeeksGraph = () => {
  const __graph_json_data = {
    ...
    seeksRelationGraph.value && seeksRelationGraph.value.setJsonData(__graph_json_data, (graphInstance) => {
